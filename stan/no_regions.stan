...
transformed parameters {
    ...
    for (m in 1:M){
      ...
      Rt[, m] = mu[m] * 2 * inv_logit(covariate[m] * beta); // Intervento della mobilità
...
model {
  ...
  mu ~ normal(2, kappa); // distribuzione di R0 tra regioni ---> cambiare
  
  gamma ~ gamma(1, 1); // effetto casuale: varianza dei beta_mk
  beta ~ normal(0,gamma); // prior su effetto fisso
